...
| 1.1.1 | 8.0.74 | 新增 AI 文本内容瑞色和联系人备注名修改能力 |
...
- 支持收集微信 Java xlog、native xlog 以及模块自身运行日志，便于排查线上问题。
- 新增 native xlog hook，覆盖 `__xlogger_Write_impl` 和 `__xlogger_VPrint_impl` 等 native 写日志入口。
- 新增日志导出分享能力，可将当前日志数据库打包为 zip 后导出或分享。
- 日志按天独立数据库文件存储，并在启动时清理过期日志文件。
- 优化日志写入流程，改为单线程异步入库，降低 hook 线程和微信主流程卡顿风险。
- 日志最小采集等级默认调整为 `VERBOSE`，便于问题排查时覆盖更多日志。

### 1.1.1

- 新增「AI 文本内容瑞色」能力，发送普通文本前可先交给 AI 优化表达。
- 支持全局开启或关闭文本瑞色功能。
- 支持配置「优化后直接发送」，开启后 AI 生成成功即发送；关闭后会弹出确认窗口。
- 支持默认提示词和自定义提示词，可按自己的表达风格调整润色要求。
- 新增幽默、专业、温柔、简洁等提示词模板，方便快速切换语气。
- 新增 AI 瑞色确认弹窗，可选择直接发送、修改后发送或原文发送。
...
| AI 文本内容瑞色配置 | AI 文本内容瑞色提示 |  |
| ![AI 文本内容瑞色配置](image/AI润色配置.png) | ![AI 文本内容瑞色提示](image/AI润色提示.png) |  |
...
在 AI 控制面板中打开「AI 文本内容瑞色」，发送普通文本消息时，AI 会先根据你的提示词优化表达。
...
如果开启「优化后直接发送」，点击发送后输入框会先清空，并提示正在瑞色，AI 生成成功后会自动发送优化后的文本。
...
在 AI 控制面板的更多设置中打开「日志回捞」后，模块会收集微信 Java/native xlog 与模块自身运行日志。再次点击「日志回捞」所在条目，可将当前日志数据库打包为 zip 并调起系统分享。

日志回捞适合用于：

- 排查模块加载、Hook、生效范围和兼容性问题。
- 排查 AI 文本内容瑞色、自动回复、消息发送等流程异常。
- 向开发者反馈问题时附带上下文日志。

日志文件仅应服务于你自己的调试和反馈需求。分享前请确认其中不包含你不希望外发的聊天上下文、联系人信息或设备信息。
...
AI 文本内容瑞色适用于你手动发送普通文本消息的场景。它会在发送前根据提示词轻微优化原文，让表达更清楚、自然或符合你选择的语气。
...
AI 文本内容瑞色不会替你决定聊天内容，也不会自动生成新的聊天任务。它只处理你已经输入、准备发送的普通文本。

### 🧰 日志回捞

日志回捞用于辅助排查运行问题。开启后可收集：

- 微信 Java xlog
- 微信 native xlog
- 模块自身运行日志

日志会按天写入独立数据库文件，并可在更多设置中打包导出。建议只在需要排查问题时开启，反馈问题前请先确认日志内容是否适合分享。
...
- 可以在更多设置中开启或关闭日志回捞。
- 开启后会收集微信 Java/native xlog 与模块自身日志。
- 点击日志回捞条目可以导出当前日志数据库压缩包。
- 日志用于问题排查，分享前请自行确认隐私内容。
...
如果开启了 AI 文本内容瑞色并关闭「优化后直接发送」，建议在确认弹窗里检查优化后的内容是否仍然符合你的原意。
...
如果遇到模块不生效、Hook 失败、AI 文本内容瑞色异常、自动回复异常或 native hook 兼容问题，可以先开启日志回捞，复现问题后导出 zip，再随问题描述一起反馈。
...
### ❔ 为什么发送文本后会弹出 AI 文本内容瑞色窗口？

说明你开启了 AI 文本内容瑞色，并关闭了「优化后直接发送」。你可以在窗口中确认 AI 优化后的内容，也可以修改后发送，或者直接发送原文。

### ❔ 为什么点击发送后输入框被清空并提示正在瑞色？

说明你开启了 AI 文本内容瑞色，并开启了「优化后直接发送」。模块会先清空输入框，等待 AI 优化完成后自动发送。如果 AI 请求失败，会尽量恢复并发送原文。
...
日志回捞由更多设置中的开关控制。建议只在排查问题时开启，完成问题反馈后可以关闭，减少不必要的日志写入。
...
- 查询微信数据只应服务于你自己的使用需求。
- 日志回捞可能包含运行上下文、联系人标识、消息片段或设备信息，分享前请自行确认。
- 外部工具服务由对应服务方提供，稳定性取决于服务本身。
- 使用 MCP 市场服务时，请确认服务来源、权限和 token 使用范围。
- 当前版本仅适配 WeChat 8.0.74，其他版本可能无法正常工作。
...
微信 Agent 是一个运行在微信内的个人 AI 助手，可以自动回复、整理联系人和群聊、修改联系人备注名、查询聊天记录、导出文件、发送消息、辅助日志回捞，并通过 MCP 扩展更多工具能力。
